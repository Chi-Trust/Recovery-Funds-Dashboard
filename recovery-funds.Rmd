...
budget_gap_filter <- filter(budget_gap, geography == "Chicago") %>%
  mutate(percentage = adopted_budget_per_treasury_report / allocation * 100)
...
```
**Note:** Percentages may be greater than 100 percent for Reasons.
<br> <br>
...
p <- ggplot(budget_gap_filter, aes(x = budget_to_allocation_gap, y = topic)) +
  geom_col(aes(text = paste("Original Allocation: ", allocation, 
                            ...
                            "<br>Percentage: ", round(percentage, 2), "%"))) +
  scale_x_continuous(
    limits = c(-5000000000, 2000000000),
    labels = scales::label_dollar(
      scale = 1 / 1e6,
      accuracy = 0.1
    )
