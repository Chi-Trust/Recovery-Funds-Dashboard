...
data_ts <- readxl::read_excel(here("data", "arpa-spending-quarterly.xlsx")) %>%
  ...
  # ) %>%
  drop_na(topic) %>%
  arrange(geography,topic,date) %>%
  group_by(geography, topic) %>%
  mutate(agg_expenditures = cumsum(total_expenditures),
         agg_budget = cumsum(adopted_budget),
         agg_obligations = cumsum(total_obligations)) %>%
  ungroup() %>%
  mutate(pct_expended_budget = agg_expenditures / agg_budget,
         pct_expended_obligations = agg_expenditures / agg_obligations )
