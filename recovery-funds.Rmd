...
ts_data <- ts_data_in %>%
  filter(completion_status != "Cancelled")  %>%
  mutate(across(matches("total|expenditure|budget"), ~ as.numeric(.x))) %>%
  arrange(project_name, desc(report_period)) %>%
...
all_periods <- expand.grid(project_name = unique(ts_data$project_name), 
...
ts_merged_data <- left_join(all_periods, ts_data, by = c("project_name", "report_period"))
...
ts_filled_data <- ts_merged_data %>%
  ...
  ungroup() %>%  
  select(project_name, geography, report_period, total_expenditures, dashboard_topic) %>%
...
dashboard_topic_budgets <- ts_data_in %>%
  ...
  select(project_name, geography, adopted_budget, dashboard_topic) %>%
...
data_ts <- left_join(ts_filled_data, dashboard_topic_budgets, by = c("dashboard_topic", "geography")) %>%
